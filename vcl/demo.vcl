sub vcl_recv {
    ...
    if (var.origin_choice == "fallback") {
      ...
      set req.http.X-Origin-Region = "us-east-2";
    } else {
      set req.backend = F_primary;
      set req.http.X-Origin-Region = "us-east-1";
    }
}
...
sub vcl_fetch {
    ...
    set beresp.http.X-Resolved-URL = bereq.url;
    # What Host we sent to S3 (useful to prove the bucket host)
    set beresp.http.X-Origin-Host = bereq.http.Host;
    # Chosen TTL string
    set beresp.http.X-Initial-TTL = var.ttl_cfg;
}
...
sub vcl_deliver {
    ...
    if (var.debug == "on") {
      ...
      if (req.http.X-Origin-Region) {
        set resp.http.X-Origin-Region = req.http.X-Origin-Region;
      }
      ...
      unset resp.http.X-Origin-Region;
      unset resp.http.X-Initial-TTL;
      unset resp.http.X-Resolved-URL;
